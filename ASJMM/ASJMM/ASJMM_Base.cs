...
{
    #region 父类抽象类
    public abstract class ASJMM_Base
    {
        ...
        protected const string sqlsupplier = @"SELECT * from BCOR_SUPPLIER where  FLAG = 1  ORDER BY CRE_TIME";//供应商
        ...
        protected const string sqlcltroute = @"SELECT TKEY, CLTNODE_NAME, CLTNODE_CODE FROM MMSMM_CLTNODE_BASE WHERE FLAG = 1 ORDER BY CRE_TIME";//物料单据采集节点基础档案
        protected const string sqlordertype = @"SELECT TKEY, ORDERTYPE_NAME, ORDERTYPE_code, BUSINESS_TYPE from MMSMM_ORDERTYPE where FLAG = 1 ORDER BY CRE_TIME";//物料单据类型 
        protected const string _sqlcltroute = @"SELECT TKEY,CLTROUTE_NAME,CLTROUTE_CODE, CASE WHEN CLTROUTE_READ_FLAG = 1 THEN '完成'  END CLTROUTE_READ_FLAG  from MMSMM_CLTROUTE where  FLAG = 1 and  CLTROUTE_READ_FLAG = 1 ORDER BY CRE_TIME";//物料单据采集节点基础档案
        ...
        {
            string sql = $@"SELECT * FROM  {DBNAME} WHERE FLAG = 1 and TKEY = ";
        ...
        /// <param name="dtlst">需要增加到dsM中的Datatable</param>
        /// <returns></returns>
