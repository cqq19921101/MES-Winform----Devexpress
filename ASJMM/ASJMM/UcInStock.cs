...
namespace ASJMM
{
    ...
    public partial class UcInStock : BaseUserControl
    {
        ...
        public void LoadData(string TKEY)
        {
            ...
            string SqlMaster = $@" SELECT * FROM MMSMM_INSTOCK WHERE FLAG = 1  AND TKEY = '{TKEY}'";
            strsql.Add(SqlMaster);//主档
            TableNames.Add("MMSMM_INSTOCK");
        ...
        public DataSet SaveFunction()
        {
            ...
            try
            {
                ...
                if (instock.TKEY == null)
                {
                    ds.Tables["MMSMM_INSTOCK"].NewRow();
                    ds.Tables["MMSMM_INSTOCK"].Rows.Add();
                }
        ...
        {
            string SqlGridView = string.Format(@"select T1.*,T2.TKEY,T2.MATERIAL_CODE,T2.MATERIAL_NAME,T2.MAPID,T2.BASE_UNIT_TKEY from MMSMM_INSTOCK_D T1 
                                   left join BCMA_MATERIAL T2 ON T1.MATERIAL_TKEY = T2.TKEY AND T1.FLAG = T2.FLAG
                                   WHERE T1.FLAG = 1 and T1.TKEY = '{0}' ", TKEY);
            MHelper.BindDataSourceForGridControl(GridItem, GrvInStock, MHelper.QueryBindGridView(SqlGridView).Ds.Tables[0]);//绑定GridControl
